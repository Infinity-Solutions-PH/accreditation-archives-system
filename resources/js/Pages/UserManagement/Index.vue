<script setup>
    ...
    // Confirmation Logic
    const confirmingAction = ref(null); // { user, type: 'approve' | 'reject' | 'delete' | 'restore' }
    ...
    const handleDelete = (user) => {
        confirmingAction.value = {
            ...
            message: `Are you sure you want to delete ${user.name}'s account? This will move the user to the trash and they will no longer be able to log in.`,
    ...
    const handleRestore = (user) => {
        confirmingAction.value = {
            ...
            message: `Are you sure you want to restore ${user.name}'s account? They will be able to log in again if their account is active.`,
    ...
    const executeAction = () => {
        ...
        if (type === 'approve' || type === 'reject') {
            ...
            const deleteRoute = activeTab.value === 'accreditors' 
                ? route('accreditors.destroy', user.id)
            ...
            const restoreRoute = activeTab.value === 'deleted_accreditors'
                ? route('accreditors.restore', user.id)
                : route('user-management.restore', user.id);
        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth">
            <div class="max-w-7xl mx-auto flex flex-col gap-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div @click="activeTab = 'deleted_users'" class="bg-white dark:bg-[#1a2234] p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-1 relative overflow-hidden group cursor-pointer hover:border-rose-500/50 transition-colors">
                        ...
                        <span class="text-slate-500 dark:text-slate-400 text-sm font-medium">Deleted Users</span>
                        <div class="flex items-baseline gap-2">
                            <span class="text-2xl font-bold text-slate-900 dark:text-white">{{ userStats.deleted_users || 0 }}</span>
                        </div>
                    </div>
                ...
                <!-- Tab Switcher -->
                <div class="flex border-b border-slate-200 dark:border-slate-700 overflow-x-auto no-scrollbar">
                    <button 
                        @click="activeTab = 'users'"
                        class="px-6 py-3 text-sm font-bold transition-all border-b-2 whitespace-nowrap"
                    ...
                    <button 
                        @click="activeTab = 'deleted_users'"
                        class="px-6 py-3 text-sm font-bold transition-all border-b-2 whitespace-nowrap"
                        :class="activeTab === 'deleted_users' ? 'text-rose-600 border-rose-600 bg-rose-50/50' : 'text-slate-500 border-transparent hover:text-slate-700 dark:hover:text-slate-300'"
                    >
                        Deleted Users ({{ userStats.deleted_users }})
                    </button>
                    <button 
                        @click="activeTab = 'deleted_accreditors'"
                        class="px-6 py-3 text-sm font-bold transition-all border-b-2 whitespace-nowrap"
                        :class="activeTab === 'deleted_accreditors' ? 'text-rose-600 border-rose-600 bg-rose-50/50' : 'text-slate-500 border-transparent hover:text-slate-700 dark:hover:text-slate-300'"
                    >
                        Deleted Accreditors ({{ userStats.deleted_accreditors }})
                    </button>
                </div>
                ...
                <div class="flex flex-col gap-4">
                    <div class="bg-white dark:bg-[#151c2b] rounded-2xl p-6 shadow-sm border border-[#e7ebf3] dark:border-gray-800 flex flex-col lg:flex-row gap-6 justify-between items-end">
                        ...
                        <div class="w-full lg:w-1/3">
                            <label class="block text-xs font-bold text-[#4c669a] uppercase tracking-wider mb-2">Search {{ activeTab.includes('accreditors') ? 'Accreditors' : 'Users' }}</label>
                        ...
                        <div class="flex flex-wrap gap-4 w-full lg:w-auto flex-1 justify-end">
                            ...
                            <!-- Status Filter (Hidden for deleted tabs) -->
                            <div v-if="!activeTab.startsWith('deleted')" class="w-full sm:w-auto min-w-[180px]">
                        ...
                        <table class="w-full text-left border-collapse transition-all duration-300" :class="{'opacity-50 grayscale-[0.5] pointer-events-none': isLoading}">
                            <thead>
                                <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 font-semibold">
                                    ...
                                    <th v-if="activeTab.includes('users')" class="p-4">Role</th>
                                    <th class="p-4">{{ activeTab.includes('users') ? 'Office' : 'Affiliation' }}</th>
                                    <th v-if="!activeTab.startsWith('deleted')" class="p-4">Status</th>
                                    <th class="p-4">{{ activeTab.includes('users') ? 'Date' : 'Access Expiry' }}</th>
                                    <th class="p-4 text-right">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-200 dark:divide-slate-700 text-sm text-slate-700 dark:text-slate-300">
                                <tr v-if="users.data.length === 0">
                                    <td colspan="7" class="p-10 text-center italic text-slate-400">No users found in this category.</td>
                                </tr>
                                <tr
                                    ...
                                    <td class="p-4 text-center text-xs text-slate-400">
                                        {{ (users.meta?.total || users.total || 0) - ((users.meta?.from || users.from || 0) + index) + 1 }}
                                    </td>
                                    <td class="p-4">
                                    ...
                                    <!-- Status (Hidden for deleted) -->
                                    <td v-if="!activeTab.startsWith('deleted')" class="p-4">
                                    ...
                                    <td class="p-4 text-xs">
                                        <div v-if="activeTab.includes('users')" class="text-slate-500 font-medium">
                                    ...
                                    <td class="p-4 text-right">
                                        <div v-if="activeTab.startsWith('deleted')" class="flex justify-end gap-1">
                                            <button @click="handleRestore(item)" class="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition-all shadow-md shadow-emerald-500/20 active:scale-95">
                                                <span class="material-symbols-outlined text-[16px]">restore</span> Restore
                                            </button>
                                        </div>
                                        <div v-else-if="item.role_status === 'pending' && activeTab === 'users'" class="flex items-center justify-end gap-1">
                                            <button 
                                        ...
                                        <div v-else class="flex items-center justify-end gap-1 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button @click="handleDelete(item)" class="p-2 rounded-lg text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-all" title="Delete">
                    ...
                    <!-- Pagination -->
                    <div v-if="(users.meta?.total || users.total) > 0" class="p-4 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                        <span class="text-sm text-slate-500 dark:text-slate-400">
                            Showing <span class="font-medium text-slate-900 dark:text-white">{{ users.meta?.from || users.from }}</span> to <span class="font-medium text-slate-900 dark:text-white">{{ users.meta?.to || users.to }}</span> of <span class="font-medium text-slate-900 dark:text-white">{{ (users.meta?.total || users.total || 0).toLocaleString() }}</span> results
                        </span>
                        <div class="flex items-center gap-1">
                            <template v-for="(link, index) in (users.meta?.links || users.links)" :key="index">
                                <Link
                                    v-if="link.url"
                                    :href="link.url"
                                    :class="[
