<script setup>
    ...
    const filteredPrograms = computed(() => {
        if (!metadata.college_id) return props.programs || [];
    ...
    const startUpload = async () => {
        ...
        try {
            ...
            if (error.response && error.response.status === 419) {
                fileError.value = 'Session expired. Please refresh the page and try again.';
            } else {
                fileError.value = 'An error occurred while initializing upload. Please try again.';
            }
        }
    }
    ...
    const uploadChunks = async () => {
        ...
        // Complete
        await api.post('/files/complete', { tmp_id: metadata.tmp_id }, {
            headers: { 
                'X-Requested-With': 'XMLHttpRequest',
                'X-CSRF-TOKEN': page.props.csrf_token 
            }
    ...
    const saveMetadata = async () => {
        ...
        try {
            await api.post('/files/update-metadata', {
            ...
            if (error.response && error.response.status === 419) {
                ...
                alert('Could not update metadata.');
            }
        } finally {
    ...
    const onCloseAttempt = async () => {
        if (uploading.value && !isCompleted.value && metadata.tmp_id) {
            const confirmAbort = confirm('File is uploading. Do you want to abort?');
            if (!confirmAbort) return;

            await api.post('/files/abort', { tmp_id: metadata.tmp_id }, {
                headers: { 
                    'X-Requested-With': 'XMLHttpRequest',
                    'X-CSRF-TOKEN': page.props.csrf_token 
                }
            ...
            // Auto-save on exit if completed
            await saveMetadata();
            router.reload({ only: ['files'], preserveScroll: true });
        }
    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @click.self="onCloseAttempt">
        <div class="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-xl shadow-2xl border overflow-hidden">
            ...
            <div class="px-6 py-4 border-b flex justify-between items-center bg-gray-50 dark:bg-gray-900">
                ...
                <button v-if="!isCompleted" @click="onCloseAttempt" class="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700">
            ...
            <div class="px-6 py-4 border-t flex justify-end gap-3">
                <button class="px-5 py-2.5 text-sm font-semibold text-text-muted-light dark:text-text-muted-dark hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors" 
                    :class="{ 'text-primary font-bold': isCompleted }"
                    type="button" @click="onCloseAttempt" v-show="!isCompleted">
                    Cancel
                </button>
                <button v-if="uploading" 
                    class="px-7 py-2.5 text-sm font-bold text-white bg-primary hover:bg-blue-700 rounded-lg shadow-md shadow-blue-500/20 transition-all flex items-center gap-2" 
                    :class="{ 'opacity-70 cursor-not-allowed': isUpdatingMetadata }"
                    type="button" 
                    :disabled="isUpdatingMetadata" 
                    @click="isCompleted ? saveAndClose() : saveMetadata()">
                    <span class="material-symbols-outlined text-[18px] animate-spin" v-show="isUpdatingMetadata">refresh</span>
                    <span class="material-symbols-outlined text-[18px]" v-show="!isUpdatingMetadata">save</span>
                    {{ isCompleted ? 'Save and Close' : 'Save' }}
                </button>
            </div>
        </div>
    </div>
